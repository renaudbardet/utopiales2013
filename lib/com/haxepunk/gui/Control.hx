...
#end
{
	...
	{
		_localX = value;
		if (_container == null) this.x = _localX + (followCamera?HXP.camera.x:0);
	...
	{
		_localY = value;
		if (_container == null) this.y = _localY + (followCamera?HXP.camera.y:0);
		return value;
	}
	 ...
	 * @return
	 */
	public function isChild(control:Entity):Bool {
		if (control == this) {
			return true;
		}
		for (c in children) {
	...
	private function get_followCamera():Bool
	{
		return _followCamera;
	}
	private function set_followCamera(value:Bool):Bool
	{
		_followCamera = value;
		if (_followCamera) {
			localX = _localX;
	 ...
	 */
	public var followCamera(get_followCamera, set_followCamera):Bool;
	...
	private var _followCamera:Bool = false;
}